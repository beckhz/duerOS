...
static struct PackageInfoOPS *s_package_info_ops = NULL;
...
int duer_ota_get_package_info(struct PackageInfo *info)
{
    int ret = DUER_OK;
    ...
    ret = duer_mutex_lock(s_lock_notifier);
    if (ret != DUER_OK) {
    ...
    ret = duer_mutex_unlock(s_lock_notifier);
    if (ret != DUER_OK) {
...
int duer_ota_register_package_info_ops(struct PackageInfoOPS *ops)
{
...
int duer_ota_notifier_state(OTAUpdater *updater, OTAState state)
{
    int ret = DUER_OK;
    baidu_json *data_json = NULL;
...
int duer_ota_notifier_event(OTAUpdater *updater, OTAEvent event, double progress)
{
    int ret = DUER_OK;
    baidu_json *data_json = NULL;
