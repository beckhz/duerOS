...
duer_speex_handler duer_speex_create(int rate)
{
    ...
    do {
        speex = (duer_speex_t *)DUER_MALLOC(sizeof(duer_speex_t));
        if (speex == NULL) {
        ...
        speex->_buffer_out = DUER_MALLOC(speex->_frame_size + sizeof(int));
        if (speex->_buffer_out == NULL) {
            DUER_LOGE("Memory overflow!!! Alloc speex buffer out failed.");
            break;
        }

        speex->_buffer_in = DUER_MALLOC(sizeof(short) * speex->_frame_size);
